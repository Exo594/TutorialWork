...
/**
 * This Network Message is sent from the server to all clients, to tell them to
 * draw a "target indicator" at the target point Typical usage: PREQUISITES:
 * have previously setup SimpleNetworkWrapper, registered the message class and
 * the handler
 *
 * 1) User creates a TargetEffectMessageToClient(targetCoordinates) 2)
 * simpleNetworkWrapper.sendToDimension(targetEffectMessageToClient); 3) network
 * code calls targetEffectMessageToClient.toBytes() to copy the message member
 * variables to a ByteBuffer, ready for sending ... bytes are sent over the
 * network and arrive at the server.... 4) network code creates
 * TargetEffectMessageToClient() 5) network code calls
 * targetEffectMessageToClient.fromBytes() to read from the ByteBuffer into the
 * member variables 6) the handler.onMessage(targetEffectMessageToClient) is
...
public class TargetEffectMessageToClient implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        try {
            ...
            targetCoordinates = Vec3.createVectorHelper(x, y, z);
